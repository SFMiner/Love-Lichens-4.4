...
func _ready():
	...
	if debug: print("Initializing combat system...")
	...
	if not combat_manager:
		...
		if debug: print("Combat Manager created")
	else:
		if debug: print("Combat Manager already exists")
	...
	if not combat_ui:
		...
		if debug: print("Combat UI created")
		
		
		call_deferred("_connect_ui_signals", combat_ui, combat_manager)
				
	initialized = true
	if debug: print("Combat system initialized for scene: ", get_tree().current_scene.name)
...
func _connect_ui_signals(combat_ui, combat_manager):
	...
	if combat_ui.has_signal("action_selected"):
		...
		if debug: print("WARNING: CombatUI missing action_selected signal")
		
	if combat_ui.has_signal("retreat_requested"):
		combat_ui.retreat_requested.connect(_on_retreat_requested.bind(combat_manager))
	else:
		if debug: print("WARNING: CombatUI missing retreat_requested signal")


func _on_action_selected(action, combat_ui, combat_manager):
	if debug: print("Combat initializer: Player selected action - " + action.type)
	...
	if combat_manager.current_combat_state != combat_manager.CombatState.PLAYER_TURN_ACTIVE:
		if debug: print("Combat initializer: ERROR - Received player action in invalid state: " + 
	...
	if player != current_character:
		if debug: print("Combat initializer: ERROR - Received player action but it's not player's turn!")
		return
	...
	if debug: print("Combat initializer: State changed to PLAYER_ACTION_PROCESSING")
	...
	if debug: print("Combat initializer: Action processing complete, ending player turn")
	...
	if combat_manager.current_combat_state == combat_manager.CombatState.PLAYER_ACTION_PROCESSING:
		...
		if debug: print("Combat initializer: WARNING - Not ending turn, state has already changed to: " + 
			str(combat_manager.current_combat_state))
