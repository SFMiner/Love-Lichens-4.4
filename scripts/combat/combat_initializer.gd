...
func _ready():
	...
	if debug: print(GameState.script_name_tag(self) + "Initializing combat system...")
	...
	if not combat_manager:
		...
		if debug: print(GameState.script_name_tag(self) + "Combat Manager created")
	else:
		if debug: print(GameState.script_name_tag(self) + "Combat Manager already exists")
	...
	if not combat_ui:
		...
		if debug: print(GameState.script_name_tag(self) + "Combat UI created")
		
		
		call_deferred("_connect_ui_signals", combat_ui, combat_manager)
				
	initialized = true
	if debug: print(GameState.script_name_tag(self) + "Combat system initialized for scene: ", get_tree().current_scene.name)
...
func _connect_ui_signals(combat_ui, combat_manager):
	...
	if combat_ui.has_signal("action_selected"):
		...
		if debug: print(GameState.script_name_tag(self) + "WARNING: CombatUI missing action_selected signal")
		
	if combat_ui.has_signal("retreat_requested"):
		combat_ui.retreat_requested.connect(_on_retreat_requested.bind(combat_manager))
	else:
		if debug: print(GameState.script_name_tag(self) + "WARNING: CombatUI missing retreat_requested signal")


func _on_action_selected(action, combat_ui, combat_manager):
	if debug: print(GameState.script_name_tag(self) + "Combat initializer: Player selected action - " + action.type)
	...
	if combat_manager.current_combat_state != combat_manager.CombatState.PLAYER_TURN_ACTIVE:
		if debug: print(GameState.script_name_tag(self) + "Combat initializer: ERROR - Received player action in invalid state: " + 
	...
	if player != current_character:
		if debug: print(GameState.script_name_tag(self) + "Combat initializer: ERROR - Received player action but it's not player's turn!")
		return
	...
	if debug: print(GameState.script_name_tag(self) + "Combat initializer: State changed to PLAYER_ACTION_PROCESSING")
	...
	if debug: print(GameState.script_name_tag(self) + "Combat initializer: Action processing complete, ending player turn")
	...
	if combat_manager.current_combat_state == combat_manager.CombatState.PLAYER_ACTION_PROCESSING:
		...
		if debug: print(GameState.script_name_tag(self) + "Combat initializer: WARNING - Not ending turn, state has already changed to: " + 
			str(combat_manager.current_combat_state))
