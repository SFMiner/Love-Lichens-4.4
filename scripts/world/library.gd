...
var camera_limit_right = 2104
var camera_limit_bottom = 1081
var camera_limit_left = 0
var camera_limit_top = 0
var zoom_factor = 1
var scene_speed_mod : float = 1

func _ready():
	var debug = scr_debug or GameController.sys_debug
	var debug_label = get_node_or_null("CanvasLayer/GameInfo")
	get_node_or_null("Node2D").set_script(load("res://scripts/world/reasearch_lab_node.gd"))
	if debug_label:
	...
	if debug: print(GameState.script_name_tag(self) + "Dorm Room scene initialized")
...
func setup_player():
	if player:
		...
		if not InputMap.has_action("interact"):
			if debug: print(GameState.script_name_tag(self) + "Adding 'interact' action to InputMap")
