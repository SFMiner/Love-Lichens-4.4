...
func _on_day_changed(old_day, new_day):
	...
	if debug: print(GameState.script_name_tag(self, _fname) + "Day changed from %d to %d" % [old_day, new_day])
	day_advanced.emit()

	# Additional day change logic here

func _on_time_of_day_changed(old_time, new_time):
	var _fname = "_on_time_of_day_changed"
	if debug: print(GameState.script_name_tag(self, _fname) + "Time of day changed")
	turn_completed.emit()
...
func _unhandled_input(event):
	...
	if InputMap.has_action("save_game") and event.is_action_pressed("save_game"):
		if debug: print(GameState.script_name_tag(self, _fname) + "Save game action detected - quick saving")
		quick_save()
	...
	if event.is_action_pressed("ui_cancel"):
		if debug: print(GameState.script_name_tag(self, _fname) + "UI cancel action detected - checking for open panels")
		...
		if inventory_panel and inventory_panel.visible:
			if debug: print(GameState.script_name_tag(self, _fname) + "Inventory panel is open, closing it")
		...
		if not panel_closed and quest_panel and quest_panel.visible:
			if debug: print(GameState.script_name_tag(self, _fname) + "Quest panel is open, closing it")
		...
		if not panel_closed:
			...
			for layer in ui_layers:
				...
				if quest_panel_in_layer and quest_panel_in_layer.visible:
					if debug: print(GameState.script_name_tag(self, _fname) + "Quest panel found in UI layer, closing it")
				...
				for child in layer.get_children():
					if child is Control and child.visible and child.name.ends_with("Panel"):
						if debug: print(GameState.script_name_tag(self, _fname) + "UI panel found: ", child.name, ", closing it")
						child.visible = false
						get_tree().paused = false
						panel_closed = true
						get_viewport().set_input_as_handled()
		...
		if not panel_closed:
			if debug: print(GameState.script_name_tag(self, _fname) + "No panels to close, toggling pause menu")
			toggle_pause_menu()
			get_viewport().set_input_as_handled()

	# Toggle inventory
	if InputMap.has_action("toggle_inventory") and event.is_action_pressed("toggle_inventory"):
		if debug: print(GameState.script_name_tag(self, _fname) + "Toggle inventory action detected")
		toggle_inventory()
		get_viewport().set_input_as_handled()
	...
	if InputMap.has_action("toggle_quest_journal") and event.is_action_pressed("toggle_quest_journal"):
		if debug: print(GameState.script_name_tag(self, _fname) + "Toggle quest journal action detected")
		toggle_quest_panel()
		get_viewport().set_input_as_handled()
	...
	if event is InputEventKey and event.keycode == KEY_K and event.pressed and not event.echo:
		...
		if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Manually triggered professor_moss talk objective")


# Start a new game
func start_new_game():
	var _fname = "start_new_game"
...
func toggle_pause_menu():
	...
	if dialog_system and dialog_system.has_method("is_dialog_active") and dialog_system.is_dialog_active():
		if debug: print(GameState.script_name_tag(self, _fname) + "Dialog active, not pausing")
		return

	# Debug info to check current path
	if debug: print(GameState.script_name_tag(self, _fname) + "Current scene path: ", current_scene_path)
	...
	if pause_menu:
		...
		if debug: print(GameState.script_name_tag(self, _fname) + "Toggling pause menu visibility: ", pause_menu.visible)
	else:
		# Create the pause menu
		if pause_menu_scene:
			if debug: print(GameState.script_name_tag(self, _fname) + "Creating pause menu")
			...
			if debug: print(GameState.script_name_tag(self, _fname) + "Pause menu scene not found or not loaded!")
			# Try to load it now
			if ResourceLoader.exists("res://scenes/ui/pause_menu.tscn"):
				pause_menu_scene = load("res://scenes/ui/pause_menu.tscn")
				if debug: print(GameState.script_name_tag(self, _fname) + "Loaded pause menu scene, try pressing Escape again")
			else:
				if debug: print(GameState.script_name_tag(self, _fname) + "Could not find pause_menu.tscn")
...
func change_scene(new_scene_path):
	...
	if get_node_or_null("/root/Game/CurrentScene"):
		...
		if error == OK:
			...
			if debug: print(GameState.script_name_tag(self, _fname) + "Failed to load scene: ", new_scene_path, " Error code: ", error)
...
func unlock_area(area_id):
	...
	if debug: print(GameState.script_name_tag(self, _fname) + "Area unlocked: ", area_id)
...
func add_knowledge(knowledge_id):
	...
	if debug: print(GameState.script_name_tag(self, _fname) + "Knowledge added: ", knowledge_id)
...
func create_test_items():
	...
	if not pickup_script:
		if debug: print(GameState.script_name_tag(self, _fname) + "ERROR: Could not load pickup_item.gd")
		return
...
func toggle_inventory():
	var _fname = "toggle_inventory"
	if debug: print(GameState.script_name_tag(self, _fname) + "Toggle inventory action detected")

	# Skip if dialog is active
	if dialog_system and dialog_system.has_method("is_dialog_active") and dialog_system.is_dialog_active():
		if debug: print(GameState.script_name_tag(self, _fname) + "Dialog active, not showing inventory")
		return
	...
	if not inventory_panel:
		...
		if ResourceLoader.exists(scene_path):
			...
			if scene:
				...
				if debug: print(GameState.script_name_tag(self, _fname) + "Inventory panel created")
			else:
				if debug: print(GameState.script_name_tag(self, _fname) + "Failed to load inventory panel scene")
				return
		else:
			if debug: print(GameState.script_name_tag(self, _fname) + "Inventory panel scene not found at path: ", scene_path)
			return

	# Toggle visibility through the panel's function
	if inventory_panel.has_method("toggle_visibility"):
		if debug: print(GameState.script_name_tag(self, _fname) + "Calling inventory panel's toggle_visibility method")
		inventory_panel.toggle_visibility()
	else:
		# Fallback to direct property toggle
		inventory_panel.visible = !inventory_panel.visible
		get_tree().paused = inventory_panel.visible
		if debug: print(GameState.script_name_tag(self, _fname) + "Directly toggled inventory panel visibility: ", inventory_panel.visible)




func on_location_entered(location_id):
	var _fname = "on_location_entered"
	if quest_system:
...
func debug_complete_quest_objective(quest_id, objective_type, target_id):
	...
	if quest_system:
		if objective_type == "visit":
			...
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Triggered location objective: ", target_id)
		elif objective_type == "talk":
			quest_system._check_talk_objectives(target_id)
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Triggered talk objective with: ", target_id)
		elif objective_type == "custom":
			quest_system.complete_custom_objective(quest_id, target_id)
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Completed custom objective: ", target_id)
		else:
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Unknown objective type: ", objective_type)
	else:
		if debug: print(GameState.script_name_tag(self, _fname) + "ERROR: QuestSystem not found")
	

# Add this new function to toggle quest journal
# Consolidated function in game_controller.gd
func toggle_quest_panel():
	var _fname = "toggle_quest_panel"
	if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: toggle_quest_panel called with J key")
	...
	if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Looking in canvas layers")
	var canvas_layers = get_tree().get_nodes_in_group("ui_layer")
	if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Found " + str(canvas_layers.size()) + " canvas layers")

	for layer in canvas_layers:
		var panel = layer.get_node_or_null("QuestPanel")
		if panel:
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Found quest panel in canvas layer")
			quest_panel = panel
			parent_canvas = layer
			break
	...
	if not quest_panel:
		quest_panel = get_node_or_null("/root/QuestPanel")
		if quest_panel:
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Found quest panel at /root/QuestPanel")

	# Create if not found anywhere
	if not quest_panel:
		if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Creating new quest panel")
		var scene_path = "res://scenes/ui/quest_panel.tscn"
		if ResourceLoader.exists(scene_path):
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Quest panel resource exists")
			var scene = load(scene_path)
			if scene:
				...
				if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Quest panel created and added to scene")
			else:
				if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Failed to load quest panel scene")
				return
		else:
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Quest panel scene not found")
			return

	# Force process mode and toggle visibility
	if quest_panel:
		if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Setting quest panel process mode")
		quest_panel.process_mode = Node.PROCESS_MODE_ALWAYS

		if parent_canvas:
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Parent canvas found, setting layer to 100")
			parent_canvas.layer = 100

		if quest_panel.visible:
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Quest panel was visible, hiding")
			quest_panel.visible = false
			get_tree().paused = false
		else:
			if debug: print(GameState.script_name_tag(self, _fname) + "DEBUG: Quest panel was hidden, showing")
			...
			if quest_panel.has_method("refresh_all_lists"):
				print(GameState.script_name_tag(self, _fname) + "DEBUG: Calling refresh_all_lists")
				quest_panel.refresh_all_lists()
	else:
		print(GameState.script_name_tag(self, _fname) + "DEBUG: Quest panel is null after all attempts")

func waiting():
	var _fname = "waiting"
	# Toggle visibility through the panel's function
	if quest_panel.has_method("toggle_visibility"):
		if debug: print(GameState.script_name_tag(self, _fname) + "Calling quest panel's toggle_visibility method")
		quest_panel.toggle_visibility()
	else:
		# Fallback to direct property toggle
		quest_panel.visible = !quest_panel.visible
		get_tree().paused = quest_panel.visible
		if debug: print(GameState.script_name_tag(self, _fname) + "Directly toggled quest panel visibility: ", quest_panel.visible)
