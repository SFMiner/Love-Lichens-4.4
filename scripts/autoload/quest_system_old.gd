...
func _ensure_quest_directory():
	...
	# Create intro quest file if it doesn't exist
	_create_default_quests()
	
func _create_default_quests():
	var intro_quest_path = "res://data/quests/intro_quest.json"
	
	if not FileAccess.file_exists(intro_quest_path):
		if debug: print("Creating default intro quest")
		
		var intro_quest = {
			"id": "intro_quest",
			"title": "First Day on Campus",
			"description": "It's your first day on campus. Explore the area and meet Professor Moss.",
			"objectives": [
				{
					"type": "visit",
					"target": "campus_quad",
					"description": "Look around the campus quad"
				},
				{
					"type": "talk",
					"target": "professor_moss",
					"description": "Talk to Professor Moss"
				}
			],
			"rewards": [
				{
				...
				{
					"type": "knowledge",
					"id": "intro_to_lichenology"
				},
				{
					"type": "relationship",
					"character": "professor_moss",
					"amount": 2
				}
			],
			"follow_up_quests": ["lichen_research"],
			"auto_start_follow_ups": false,
			"completed": false
		}
		
		# Save the quest file
		var file = FileAccess.open(intro_quest_path, FileAccess.WRITE)
		if file:
			file.store_string(JSON.stringify(intro_quest, "  "))
			file.close()
			if debug: print("Created intro quest file successfully")
		else:
			if debug: print("ERROR: Failed to create intro quest file")
...
# In quest_system.gd, improve the _check_talk_objectives function:
		...
		if quest.has("objectives"):
			for i in range(quest.objectives.size()):
				...
				if objective.type == "visit" and objective.target == location_id and not objective.completed:
					if debug: print("Found matching visit objective for ", location_id)
					objective.completed = true
					any_objectives_found = true
...
func complete_custom_objective(quest_id, objective_tag):
	...
	if quest.has("objectives"):
		for i in range(quest.objectives.size()):
			...
			if objective.type == "custom" and objective.target == objective_tag and not objective.completed:
				objective.completed = true
...
# In quest_system.gd, modify the on_area_visited function
func on_area_visited(area_name, location_id=""):
	...
	# Check for multi-visit objectives
	_check_multi_area_objectives()  # Add this line
...
func _check_area_visit_objectives(area_name, location_id=""):
	...
	# Check for area-specific objectives
	for quest_id in active_quests:
		var quest = active_quests[quest_id]
		...
		if quest.has("objectives"):
			for i in range(quest.objectives.size()):
				...
				# Check for objectives that require visiting this specific area
				if objective.type == "visit" and objective.target == area_name and not objective.completed:
	...
	# Check for location exploration completion
	if location_id.length() > 0 and area_exploration.has(location_id):
		# Check if we have visited all areas in this location
		if area_exploration[location_id].visited_areas.size() >= 4:  # Assuming 4 areas needed for campus_quad
			_check_explore_objectives(location_id)
	
	if debug and not any_objectives_found:
		print("No area visit objectives found for: ", area_name)
		print("No direct area visit objectives found for: ", area_name)
	...
	for quest_id in active_quests:
		...
		if quest.has("objectives"):
			for i in range(quest.objectives.size()):
				...
				# Check for "visit X areas in Y location" objectives
				if objective.type == "multi_visit" and objective.has("areas") and not objective.completed:
					var visited_count = 0
					var required_count = objective.areas.size()
					
					# If a minimum count is specified, use that instead
					if objective.has("min_areas"):
						required_count = objective.min_areas
					
					# Count how many required areas have been visited
					for area in objective.areas:
						if visited_areas.has(area):
							visited_count += 1
						# Also check if we have location-specific visit entries
						elif visited_areas.has("campus_quad:" + area):
							visited_count += 1
					
					if debug: print("Multi-visit progress: ", visited_count, "/", required_count, " for quest ", quest_id)
					
					# Update progress
					if objective.has("progress"):
						var old_progress = objective.progress
						objective.progress = visited_count
						
						if old_progress != visited_count:
							objective_updated.emit(quest_id, i, visited_count, required_count)
					else:
						objective.progress = visited_count
						objective_updated.emit(quest_id, i, visited_count, required_count)
					
					# Check if enough areas have been visited
					if visited_count >= required_count:
						objective.completed = true
						quest_updated.emit(quest_id)
						
						if debug: print("Completed multi-visit objective: ", 
							objective.description if objective.has("description") else "Unnamed objective")
